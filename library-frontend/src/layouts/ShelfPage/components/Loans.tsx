...
export const Loans = () => {
	...
	useEffect(() => {
		const fetchUserCurrentLoans = async () => {
			if (authState && authState.isAuthenticated) {
				const url = `http://localhost:8200/api/books/secure/currentloans`;
	...
	async function returnBook(bookId: number) {
		const url = `http://localhost:8200/api/books/secure/return/?bookId=${bookId}`;
		const requestOptions = {
			method: 'PUT',
			headers: {
				Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
	...
	async function renewLoan(bookId: number) {
		const url = `http://localhost:8200/api/books/secure/renew/loan/?bookId=${bookId}`;
		const requestOptions = {
			method: 'PUT',
			headers: {
				Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
