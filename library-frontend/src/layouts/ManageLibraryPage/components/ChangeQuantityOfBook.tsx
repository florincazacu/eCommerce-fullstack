...
export const ChangeQuantityOfBook: React.FC<{book: BookModel, deleteBook: any}> = (props, key) => {
	...
	async function increaseQuantity() {
		const url = `http://localhost:8200/api/admin/secure/increase/book/quantity?bookId=${props.book.id}`;
	...
	async function decreaseQuantity() {
		const url = `http://localhost:8200/api/admin/secure/decrease/book/quantity?bookId=${props.book.id}`;
		const requestOptions = {
			method: 'PUT',
			headers: {
				Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
	...
	async function deleteBook() {
		const url = `http://localhost:8200/api/admin/secure/delete/book/?bookId=${props.book.id}`;
		const requestOptions = {
			method: 'DELETE',
			headers: {
				Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
	...
	return (
		<div className='card mt-3 shadow p-3 mb-3 bg-body rounded'>
			<div className='row g-0'>
				...
				<button className='m-1 btn btn-md main-color text-white' onClick={increaseQuantity}>Add Quantity
				</button>
